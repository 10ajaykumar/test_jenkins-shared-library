        ...
        stages {
            ...
            stage('Build & Push Images') {
                ...
                steps {
                    container('buildkit') {
                        script {
                            updateStageStatus("Build & Push Images", "BuildKit building: ${env.CHANGED_SERVICES}")
                            ...
                            sh """
                                ...
                                for i in \$(seq 1 20); do
                                    ...
                                    echo "Waiting for buildkitd... attempt \$i"
                                    sleep 3
                                done
                            """
                            ...
                            serviceConfig.services.findAll { changedList.contains(it.name) }.each { service ->
                                ...
                                def fullImage  = "${ECR_REGISTRY}/${ecrRepo}:${IMAGE_TAG}"
                                def latestImage = "${ECR_REGISTRY}/${ecrRepo}:latest"

                                sh """
                                    export BUILDKIT_HOST=unix:///run/user/1000/buildkit/buildkitd.sock

                                    # Authenticate to ECR via IRSA — no credentials stored in pod
                                    aws ecr get-login-password --region ${AWS_DEFAULT_REGION} | \
                                        buildctl build \
                                            --frontend dockerfile.v0 \
                                            --opt filename=Dockerfile \
                                            --local context=${WORKSPACE}/application/${service.path} \
                                            --local dockerfile=${WORKSPACE}/application/${service.path} \
                                            --output type=image,name=${fullImage},push=true \
                                            --output type=image,name=${latestImage},push=true \
                                            --export-cache type=registry,ref=${ECR_REGISTRY}/${ecrRepo}:cache \
                                            --import-cache type=registry,ref=${ECR_REGISTRY}/${ecrRepo}:cache
                                """
                            }
                        }
                    }
                    ...
                    script {
                        ...
                        lock(resource: "gitops-${env.DEPLOY_ENV}") {
                            ...
                            withCredentials([
                                ...
                                dir('gitops-repo') {
                                    deleteDir()
                                    git(url: GITOPS_REPO, branch: "main", credentialsId: GIT_CREDENTIALS)
